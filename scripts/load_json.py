...
""",
"venue_ins": """
INSERT INTO venue (cinema_id, name, address)
VALUES (%s,%s,%s)
...
def ensure_cinema_and_venue(cur, cinema_name, venue_name, cinema_website=None, venue_addr=None):
    ...
    upsert(cur, SQL["venue_ins"], (cid, venue_name, venue_addr))
    row = fetch_one(cur, "SELECT id FROM venue WHERE cinema_id=%s AND name=%s", (cid, venue_name))
    vid = row[0]
    return cid, vid
...
def ensure_person(cur, name, imdb=None, tmdb=None):
    ...
    row = fetch_one(cur, "SELECT id FROM person WHERE normalized_name=LOWER(%s)", (norm_title(name),))
    return row[0]

def link_directors(cur, film_id, director_field):
    if not director_field: return
...
def parse_dt_cinematheque(date_str: str, time_str: str) -> datetime:
    ...
    try:
        ...
        dt_local = datetime.strptime(f"{date_str} {time_str}", "%Y-%B-%d %I:%M %p")
    except ValueError:
...
def load_cinematheque(cur, path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        rows = json.load(f)
    cid, vid = ensure_cinema_and_venue(cur, cinema, "The Cinematheque", "https://thecinematheque.ca")
    upsert(cur, SQL["raw_import_ins"], (cid, json.dumps(rows, ensure_ascii=False)))
...
def load_viff(cur, path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        rows = json.load(f)
    cid, viff_cinema = ensure_cinema_and_venue(cur, cinema, "VIFF Centre - VIFF Cinema", "https://viff.org")
    _,   lochmaddy  = ensure_cinema_and_venue(cur, cinema, "VIFF Centre - Lochmaddy Studio Theatre", "https://viff.org")
    upsert(cur, SQL["raw_import_ins"], (cid, json.dumps(rows, ensure_ascii=False)))
    venue_map = {
        "VIFF Centre - VIFF Cinema": viff_cinema,
        "VIFF Centre - Lochmaddy Studio Theatre": lochmaddy
    }
    ...
    for r in rows:
        ...
        for st in r.get("showtimes", []):
            venue_id = venue_map.get(st.get("venue")) or viff_cinema
            dt_local = parse_dt_viff(st.get("date",""), st.get("time",""), screening_year)
            start_utc = to_utc(dt_local)
            end_utc = guess_end(start_utc, runtime)
            upsert(cur, SQL["screening_ins"], (
...
def load_rio(cur, path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        rows = json.load(f)
    cid, vid = ensure_cinema_and_venue(cur, cinema, "Rio Theatre", "https://riotheatre.ca")
    upsert(cur, SQL["raw_import_ins"], (cid, json.dumps(rows, ensure_ascii=False)))
    ...
    for r in rows:
        ...
        screening_year = datetime.now(LOCAL_TZ).year
        for st in r.get("showtimes", []):
            dt_local = parse_dt_rio(st.get("date",""), st.get("time",""), screening_year)
            start_utc = to_utc(dt_local)
            end_utc = guess_end(start_utc, runtime)
            upsert(cur, SQL["screening_ins"], (
                film_id, vid, start_utc, end_utc, runtime,
