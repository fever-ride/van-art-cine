...
log = get_logger("merge_duplicate_films")
...
def merge_film_group(conn, keep_id: int, merge_ids: List[int], dry_run: bool = False) -> None:
    ...
    for mid in merge_ids:
        ...
        if not merge_details:
            log.warning(f"  {prefix}Film ID {mid} already missing; skipping.")
            continue
...
def main() -> None:
    ...
    try:
        ...
        log.info("Step 1: Finding duplicate films by imdb_id...")
        imdb_dupes = find_film_duplicates_by_field(conn, "imdb_id")
        log.info(f"Found {len(imdb_dupes)} duplicate groups by imdb_id")
        ...
        log.info("Step 2: Finding duplicate films by tmdb_id...")
        tmdb_dupes = find_film_duplicates_by_field(conn, "tmdb_id")
        log.info(f"Found {len(tmdb_dupes)} duplicate groups by tmdb_id")
        ...
        if dry_run:
            log.info(
                f"DRY RUN COMPLETE — would merge {total_merged} duplicate film "
                f"records. Run without --dry-run to apply changes."
            )
        else:
            conn.commit()
            log.info(f"SUCCESS: Merged {total_merged} duplicate film records")
